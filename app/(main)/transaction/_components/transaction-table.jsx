...
export function TransactionTable({ transactions, accounts }) {
  ...
  const urlSearchFilter = searchParams.get("search");
  const urlRecurringFilter = searchParams.get("recurring");

  // Memoized filtered and sorted transactions
  const filteredAndSortedTransactions = useMemo(() => {
    let result = [...transactions];

    // Apply URL-based account filter
    if (urlAccountFilter && urlAccountFilter !== "all") {
      result = result.filter((transaction) => transaction.accountId === urlAccountFilter);
    }

    // Apply URL-based type filter
    ...
    if (urlCategoryFilter && urlCategoryFilter !== "all") {
      result = result.filter((transaction) => transaction.category === urlCategoryFilter);
    }

    // Apply URL-based date filters
    if (urlDateFrom) {
    ...
    if (urlSearchFilter) {
      const searchLower = urlSearchFilter.toLowerCase();
      result = result.filter((transaction) =>
        transaction.description?.toLowerCase().includes(searchLower)
      );
    }

    // Apply URL-based recurring filter
    if (urlRecurringFilter && urlRecurringFilter !== "all") {
      result = result.filter((transaction) => {
        if (urlRecurringFilter === "recurring") return transaction.isRecurring;
        return !transaction.isRecurring;
      });
    }

    // Apply local search filter (for backward compatibility)
    if (searchTerm) {
    ...
    // Apply local type filter (for backward compatibility)
    if (typeFilter) {
      result = result.filter((transaction) => transaction.type === typeFilter);
    }

    // Apply local recurring filter (for backward compatibility)
    if (recurringFilter) {
      result = result.filter((transaction) => {
    ...
    return result;
  }, [transactions, searchTerm, typeFilter, recurringFilter, sortConfig, urlAccountFilter, urlTypeFilter, urlCategoryFilter, urlDateFrom, urlDateTo, urlSearchFilter, urlRecurringFilter]);
